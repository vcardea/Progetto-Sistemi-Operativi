...
/**
 * @brief Conteggio dei processi bloccati in attesa di I/O o del timer (soft-blocked).
 * Incrementato quando un processo si blocca su un semaforo di device/timer,
 * decrementato quando viene sbloccato o terminato.
 */
...
/**
 * @brief Puntatore al PCB del processo attualmente in esecuzione (in stato "running").
 * Se `NULL`, nessun processo è in esecuzione.
 */
 ...
 * L'ordine è: 8 dischi, 8 flash, 8 network, 8 stampanti, 8 terminali (ricezione),
 * 8 terminali (trasmissione), 1 pseudo-clock.
 */
...
/**
 * @brief Gestore per le eccezioni relative alla TLB (Translation Lookaside Buffer).
 *
